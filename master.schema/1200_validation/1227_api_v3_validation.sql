...
===============================================================================
 ...
 Purpose:        Validate the complete v3 database API contract, additive
                 lifecycle state, concurrency helpers, exact inventory imports,
                 public visibility reads, and privileged administrator separation.
...
SELECT app.assert_true(
    ...
    AND to_regprocedure('api.import_inventory_normalized(jsonb)') IS NOT NULL
    ...
    'One or more v3 API routines are missing'
);
...
SELECT app.assert_true(
    ...
    AND has_function_privilege('brktrkr_api','api.import_inventory_normalized(jsonb)','EXECUTE')
    ...
    'brktrkr_api lacks one or more reviewed v3 routine grants'
);
...
SELECT app.assert_true(
    ...
    AND NOT has_function_privilege('PUBLIC','api.import_inventory_normalized(jsonb)','EXECUTE')
    ...
    'PUBLIC can execute a v3 API routine'
);
...
SELECT app.assert_true(
    ...
    AND NOT (SELECT anonymous_safe FROM app.runtime_api_allowlist WHERE routine_signature='api.import_inventory_normalized(jsonb)'),
    'Anonymous-safe API classification is incorrect for v3 read/mutation routines'
);
...
SELECT app.assert_true(
    (SELECT count(*) FROM app.runtime_api_allowlist)=29,
    'v3 runtime API allowlist cardinality is not 29'
);

\echo '[PASS] 1227_api_v3_validation.sql v1.3.2'
