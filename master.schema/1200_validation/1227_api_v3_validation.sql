...
===============================================================================
 ...
 Purpose:        Validate the complete v3 database API contract, additive
                 lifecycle state, concurrency helpers and privileged separation.
...
SELECT app.assert_true(
    ...
    AND to_regprocedure('api.admin_finance_operation(text,jsonb,jsonb)') IS NOT NULL,
...
SELECT app.assert_true(
    NOT has_function_privilege('brktrkr_api','api.admin_finance_operation(text,jsonb,jsonb)','EXECUTE')
    AND has_function_privilege('brktrkr_admin','api.admin_finance_operation(text,jsonb,jsonb)','EXECUTE'),
...
SELECT app.assert_true(
    ...
    AND NOT has_function_privilege('PUBLIC','api.admin_finance_operation(text,jsonb,jsonb)','EXECUTE'),
...
SELECT app.assert_true(
    (SELECT count(*) FROM app.runtime_api_allowlist)=27,
    'v3 runtime API allowlist cardinality is not 27'
);

\echo '[PASS] 1227_api_v3_validation.sql'
