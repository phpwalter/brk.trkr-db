...
def resolve_run(conn):
    with conn.cursor() as cur:
            ...
            WHERE s.source_code = 'REBRICKABLE'
              ...
              AND EXISTS (
                  ...
                  WHERE d.source_run_id = r.source_run_id
                    AND d.dataset_name = 'inventories'
        ...
        row = cur.fetchone()
        if row is None:
            raise RuntimeError("no validated Phase 5A source run found")
        return row[0]


def progress(conn, run_id):
    with conn.cursor() as cur:
        cur.execute("""
            ...
            FROM import.phase5b_progress(%s)
        """, (run_id,))
        return cur.fetchall()
...
def show(conn, run_id):
    print("")
    print("-------------------------------------------------------------------------------")
    print(" Durable Phase 5B checkpoint status")
    print("-------------------------------------------------------------------------------")
    current = None
    for r in progress(conn, run_id):
        ...
        print(
            f"      [{so-200}.{sso}] {sub:<28} {status:<9} "
            f"{done:>10,} / {total:<10,} {pct_s} batches={batches}"
        )
    print("")
...
def main():
    ...
    try:
        with psycopg.connect(
            ...
            print("==============================================================================")
            print(f" BrickTrackr Rebrickable Phase 5B - Checkpointed Reconcile v{VERSION}")
            print("==============================================================================")
            ...
            with conn.transaction():
                with conn.cursor() as cur:
                    cur.execute(
                        "SELECT import.phase5b_initialize(%s,%s)",
                        (run_id, a.restart),
                    )
                    init = cur.fetchone()[0]
            ...
            print(
                ...
                f"inventories={init['inventory_rows']:,}, "
                f"parts={init['inventory_part_rows']:,}, "
                f"sets={init['inventory_set_rows']:,}, "
                f"minifigs={init['inventory_minifig_rows']:,}"
            )
            show(conn, run_id)

            while True:
                checkpoint = first_incomplete(conn, run_id)
                if checkpoint is None:
                    break

                step, sub, so, sso, done, total, batches = checkpoint
                pct = 100.0 * done / total if total else 100.0

                print(
                    f"[RUN] [{so-200}.{sso}] {step} / {sub} "
                ...
                with conn.transaction():
                    with conn.cursor() as cur:
                        cur.execute(
                            "SELECT import.phase5b_run_checkpoint(%s,%s,%s,%s)",
                            (run_id, step, sub, a.batch_size),
                        )
                        result = cur.fetchone()[0]

                elapsed = time.monotonic() - t0
                rp = int(result.get("rows_processed") or 0)
                re = int(result.get("rows_expected") or 0)
                pc = result.get("percent_complete")
                pc = 100.0 if pc is None else float(pc)
            ...
            print("==============================================================================")
            print(" [PASS] Rebrickable Phase 5B checkpointed reconciliation completed")
            print(f" [INFO] Source run: {run_id}")
        ...
        print(
            f"[FAIL] Phase 5B: {type(exc).__name__}: {exc}",
            file=sys.stderr,
        )
        print(
            "[INFO] Completed batches remain committed; resume after correction.",
            file=sys.stderr,
        )
        return 1
