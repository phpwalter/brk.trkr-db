...
function Write-SystemSummary {
    ...
    $sql = @"
SELECT import.get_system_summary()::text;
"@

    $summary = Invoke-PsqlScalar -Sql $sql
    Write-Log "SUMMARY" ("{0} {1}" -f $Context, $summary)
}
    ...
    foreach ($phase in $phaseOrder[$startIndex..($phaseOrder.Count - 1)]) {
        switch ($phase) {
            ...
            "PHASE3B" {
                ...
                Write-SystemSummary -Context "PHASE3B"
            ...
            "PHASE4B" {
                ...
                Write-SystemSummary -Context "PHASE4B"
            ...
            "PHASE5B" {
                ...
                $stickerResult = Invoke-PsqlScalar -Sql @"
SELECT import.reconcile_rebrickable_sticker_sheets('$currentRunId'::uuid)::text;
"@
                Write-Log "PHASE5B" ("Sticker manifest reconciliation {0}" -f $stickerResult)
                ...
                Write-SystemSummary -Context "PHASE5B"
            ...
            "PHASE6B" {
                ...
                Write-SystemSummary -Context "PHASE6B"
